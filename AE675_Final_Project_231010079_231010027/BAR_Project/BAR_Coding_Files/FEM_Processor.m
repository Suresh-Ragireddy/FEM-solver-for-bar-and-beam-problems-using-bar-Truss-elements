function [K_global_D_bc,Fv,S,X] = FEM_Processor(P,e,L,a,b,c,f)
    ...
    % Ensure c_x is a column vector
    c_x = c_x(:);  % This reshapes c_x to be a column vector if it isn't already
    
    % Reshape the tensor KL3_Ten3_global for matrix multiplication
    % We want to multiply such that each slice i,j,: gets multiplied by c_x
    tempMatrix = reshape(permute(KL3_Ten3_global, [3, 1, 2]), global_matrix_size, []);
    
    % Multiply
    tempResult = tempMatrix' * c_x;
    
    % Reshape the result back to the desired matrix form
    KL3_global = reshape(tempResult, global_matrix_size, global_matrix_size);


% % Taking Hours to run ineffiect by using this nested loops
